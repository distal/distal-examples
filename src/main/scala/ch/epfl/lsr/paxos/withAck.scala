 ...
 */
abstract class AckingLegislator(val ID :LegislatorName, val ledger :Ledger, president : => LegislatorName) extends DSLProtocol { 
  import ledger._

  val MAJORITY = ALL.size/2 + 1

  private val proposedValue = collection.mutable.Map[DecreeNr, Decree]()
  ...
  UPON RECEIVING BeginBallot WITH { _.b == nextBal} DO { 
    msg =>
      proposedValue += (msg.d.n -> msg.d)
      if(SENDER != LOCATION) {  
	// remote
	val vote = Vote(msg.b, msg.d)
	prevVote += vote
	| FORWARD Ack(msg.b, msg.d.n) TO this 
	| SEND Ack(msg.b, msg.d.n) TO ALL
      }
    | DISCARD msg
  }
  ...
  UPON RECEIVING Ack WITH { _.b == nextBal } SAME { _.dn } TIMES MAJORITY DO { 
    msgs =>
      val msg = msgs.head
      val d = proposedValue(msg.dn)

      note(d)
      decided(d)

      proposedValue -= (msg.dn)
      prevVote -= Vote(msg.b, d)
  ...
  def isDecided(ack :Ack) = { 
    alreadyDecided(ack.dn)
  }

  UPON RECEIVING Ack WITH { _.b == nextBal } WITH isDecided DO { 
    msg => 
      | DISCARD msg
  }

  UPON RECEIVING Ack WITH { _.b != nextBal } DO { 
    msg => 
      | DISCARD msg
  }
